...
AppTitle "SCP - Containment Breach Classic Mod Reborn "+ModNumber$
...
Function UpdateConsole()
	...
	If ConsoleOpen Then
		...
		AAText (x + 30, y + height - 35, ConsoleInput)
...
;Next
;For i = 10 To 12
;	IntroSFX(i) = LoadSound("SFX\Room\Intro\elec" + (i - 9) + ".ogg")
;Next
...
Function UseDoor(d.Doors, showmsg%=True, playsfx%=True)
	...
	ElseIf d\KeyCard < 0
		...
		If temp <> 0 Then
			...
			ElseIf (SelectedItem\itemtemplate\tempname = "scp005") Then
				PlaySound KeyCardSFX1
				Msg = "You hold the key next to the scanner"	
			EndIf
			MsgTimer = 70 * 10
		Else
			If showmsg = True Then 
...
Function DrawEnding()
	...
	If (Not UsedConsole) Then AchvConsole = True
	...
	If EndingTimer<-200 Then
		...
		Else
			...
			If EndingTimer < -1000 And EndingTimer > -2000
				...
				AAText(x + width / 2 + 40, y + 20, "THE END", True)
				AASetFont Font1
				...
				If AchievementsMenu=0 Then 
					...
					AAText x, y, "SCPs encountered: " +temp
					AAText x, y+20, "Rooms found: " + roomsfound+"/"+roomamount
					AAText x, y+40, "Documents discovered: " +docsfound+"/"+docamount
					AAText x, y+60, "Items refined in SCP-914: " +RefinedItems			
					
					DrawTick (x, y + 130, AchvConsole, True)
					AAText x+40, y+130, "Didn't use console commands"
					DrawTick (x, y + 160, Contained106, True)
					AAText x+40, y+160, "Contained SCP-106"
					DrawTick (x, y + 190, AchvPD, True)			
					AAText x+40, y+190, "Escaped the pocket dimension"
					DrawTick (x, y + 220, AchvTesla, True)				
					AAText x+40, y+220, "Lured SCP-106 through a Tesla Gate"
				
					DrawTick (x, y + 250, AchvSNAV, True)				
					AAText x+40, y+250, "Obtained S-NAV Ultimate"
					DrawTick (x, y + 280, AchvOmni, True)				
					AAText x+40, y+280, "Obtained Key Card Omni"			
				
					DrawTick (x, y + 310, AchvMaynard, True)			
					AAText x+40, y+310, "Entered Dr. Maynard's office"
					DrawTick (x, y + 340, AchvHarp, True)			
					AAText x+40, y+340, "Entered Dr. Harp's office"
...
;					EndIf
					...
					If DrawButton(x-110,y,300,60,"MAIN MENU", True)
						ShouldPlay = 24
						NowPlaying = ShouldPlay
						For i=0 To 9
...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		the_yaw# = ( ( mouse_x_speed_1# + mouse_x_speed_2# + mouse_x_speed_3# + mouse_x_speed_4# + mouse_x_speed_5# ) / 7.0 ) * mouselook_x_inc#
		the_pitch# = ( ( mouse_y_speed_1# + mouse_y_speed_2# + mouse_y_speed_3# + mouse_y_speed_4# + mouse_y_speed_5# ) / 7.0 ) * mouselook_y_inc#
		;^^^^^^
					...
					If SelectedItem\state > 0 Then
						...
						Else
							...
							If SelectedItem\state <= 100 Then
								...
								For i = 1 To 5
									If Ceil(SelectedItem\state / 20.0) > 5 - i Then
										Rect(x + (250 - i * 6), y, 40 - i * 6, 5)
									Else
										Rect(x + (250 - i * 6), y, 40 - i * 6, 5, False)
									EndIf
							...
							Else
								...
								AAText(x+177, y+16, Int(SelectedItem\state2+1),True,True)
							EndIf
						...
						If SelectedItem\state > 0 And (Rnd(CoffinDistance + 15.0) > 1.0 Or PlayerRoom\RoomTemplate\Name <> "coffin") Then
							...
							If (MilliSecs2() Mod 1000) > 300 Then
								If SelectedItem\itemtemplate\name <> "S-NAV 310 Navigator" And SelectedItem\itemtemplate\name <> "S-NAV Navigator Ultimate" Then
									...
									AAText(x, y + height / 2 - 40, "COULDN'T CONNECT", True)
							...
							If SelectedItem\itemtemplate\name = "S-NAV Navigator Ultimate" And (MilliSecs2() Mod 600) < 400 Then
								...
								If Curr106<>Null Then
									...
									If dist < 8.0 * 4 Then
										...
										AAText(x - width / 2 + 20, y - height / 2 + 40, "SCP-106")
										SCPs_found% = SCPs_found% + 1
									EndIf
								EndIf
								If Curr096<>Null Then 
									...
									If dist < 8.0 * 4 Then
										...
										AAText(x - width / 2 + 20, y - height / 2 + 40, "SCP-096")
										SCPs_found% = SCPs_found% + 1
									EndIf
								EndIf
									...
									If np\NPCtype = NPCtype049
										...
										If dist < 8.0 * 4 Then
											If (Not np\HideFromNVG) Then
												...
												AAText(x - width / 2 + 20, y - height / 2 + 40 + (20*SCPs_found), "SCP-049")
												SCPs_found% = SCPs_found% + 1
											EndIf
										EndIf
										Exit
								...
								If PlayerRoom\RoomTemplate\Name = "coffin" Then
									If CoffinDistance < 8.0 Then
										...
										AAText(x - width / 2 + 20, y - height / 2 + 40, "SCP-895")
									EndIf
								EndIf
							End If
							...
							If SelectedItem\itemtemplate\name = "S-NAV Navigator" Then Color(255, 0, 0)
							If SelectedItem\state <= 100 Then
								;AAText (x - 140, y - 120, "BATTERY")
								AAText (x - 60, y - 20, "BATTERY")
								For i = 1 To 5
									If Ceil(SelectedItem\state / 20.0) > 5 - i Then
...
Function DrawMenu()
	...
	If MenuOpen Then
		...
		AAText x, y+20, "Name: [REDACTED]"
		
		AAText x, y+50,	"Save: "+CurrSave
		AAText x, y+70, "Map seed: "+RandomSeed
		...
		If KillTimer >= 0 Then
			AASetFont Font2
			AAText(x + width / 2, y + 20, "PAUSED", True)
			AASetFont Font1
		Else
			AASetFont Font2
			AAText(x + width / 2, y + 20, "YOU DIED", True)
			AASetFont Font1
		End If
		...
		If SelectedDifficulty\saveType = SAVEONQUIT Or SelectedDifficulty\saveType = SAVEANYWHERE Then ;euclid-mode
			If KillTimer >= 0 Then ;ei ole kuollut			
				...
				Else
					...
					Color 50,50,50
					AAText(x + width / 2, y + 154 + 80 + 30, "Save & quit", True, True)
				EndIf
			...
			Else ;kuoli
				...
				Else
					...
					Color 50,50,50
					AAText(x + width / 2, y + 154 + 30, "Load game", True, True)
				EndIf
