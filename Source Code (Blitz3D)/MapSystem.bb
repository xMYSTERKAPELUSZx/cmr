...
;			EntityParent dp\cam,fr\Forest_Pivot
		...
		Case "exit1"
			...
			r\RoomDoors[4] = CreateDoor(r\zone, r\x, 0, r\z - 320.0 * RoomScale, 0, r, False, True, 3)
			r\RoomDoors[4]\dir = 1 : r\RoomDoors[4]\AutoClose = False : r\RoomDoors[4]\open = False
		...
		Case "room079"
			;[Block]
			d = CreateDoor(r\zone, r\x, -448.0*RoomScale, r\z + 1136.0 * RoomScale, 0, r, False,True, 3)
			...
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 386.0*RoomScale, -448.0*RoomScale, r\z + 3086.0 * RoomScale, 0, r, False, True, 3)
			r\RoomDoors[0]\dir = 1 : r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			PositionEntity(r\RoomDoors[0]\buttons[1], r\x + 730.0 * RoomScale, -250*RoomScale, r\z + 3236.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[0]\buttons[0],0,-90-90,0,True)
			PositionEntity(r\RoomDoors[0]\buttons[0], r\x + 730.0 * RoomScale, -240*RoomScale, r\z + 2740.0 * RoomScale, True)
			...
			PositionEntity (r\Objects[0], r\x -526.0*RoomScale, -560.0*RoomScale, r\z + 4770.0*RoomScale, True)
			EntityParent(r\Objects[0], r\obj)
			TurnEntity r\Objects[0],0,730,0,True
		...
		Case "room2testroom2"
			...
			d = CreateDoor(r\zone, r\x - 240.0 * RoomScale, 0.0, r\z + 640.0 * RoomScale, 90, r, False, False)
			d\AutoClose = False : d\open = False		
		...
		Case "room2tunnel"
			...
			r\RoomDoors[1] = CreateDoor(r\zone, r\x - 2328.0 * RoomScale, -2496.0 * RoomScale, r\z - 656.0 * RoomScale, 90, r, False)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False	
			...
			r\RoomDoors[3] = CreateDoor(r\zone, r\x + 2360.0 * RoomScale, -2496.0 * RoomScale, r\z + 656.0 * RoomScale, 90, r, False)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False		
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "testroom"
			...
			;CreateDoor(0, r\x + 720.0 * RoomScale, 0, r\z, 0, r, False, 2, 0, "ABCD")
		...
		Case "start"
			;[Block]
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 288.0 * RoomScale, 0, r\z + 384.0 * RoomScale, 90, r, True, True)
		...
		Case "173"
			...
			r\RoomDoors[0] = CreateDoor(r\zone, EntityX(r\obj), 0, EntityZ(r\obj) - 4.0, 0, r, False)
			r\RoomDoors[0]\locked = True
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			
			r\RoomDoors[1] = CreateDoor(r\zone, EntityX(r\obj) + 288.0 * RoomScale, 0, EntityZ(r\obj) + 384.0 * RoomScale, 90, r, True, True)
			...
			r\RoomDoors[5] = CreateDoor(r\zone, r\x - 5760.0 * RoomScale, 0, r\z + 320.0 * RoomScale, 0, r, True)
			r\RoomDoors[5]\AutoClose = False : r\RoomDoors[5]\open = True : r\RoomDoors[5]\locked = True	
			
			d.Doors = CreateDoor(r\zone, r\x - 5760 * RoomScale, 0, r\z + 1216 * RoomScale, 0, r, False)
			d\locked = True
...
Function CreateMap()
	...
	SetRoom("room1archive",ROOM1,Floor(0.5*Float(Room1Amount)),min_pos,max_pos)
	SetRoom("room205", ROOM1, Floor(0.6*Float(Room1Amount)),min_pos,max_pos)
	SetRoom("005",ROOM1,Room1Amount[0]+Floor(0.3*Float(Room1Amount)),min_pos,max_pos)
	SetRoom("room2test1074",ROOM2,Floor(0.9*Float(Room2Amount[0])),min_pos,max_pos)
	SetRoom("statueroom",ROOM1,Room1Amount[0]+Floor(0.3*Float(Room1Amount)),min_pos,max_pos)
	SetRoom("conference",ROOM1,Room1Amount[0]+Floor(0.3*Float(Room1Amount)),min_pos,max_pos)
	...
	MapRoom(ROOM2, Floor(0.3*Float(Room2Amount[0]))) = "room2testroom2"	
	MapRoom(ROOM2, Floor(0.6*Float(Room2Amount[2]))) = "room2scps"
	SetRoom("room2storage", ROOM2, Floor(0.3*Float(Room2Amount)),min_pos,max_pos)
	...
	MapRoom(ROOM2, Floor(0.9*Float(Room2Amount[0]))) = "room012"
	SetRoom("room2scps2",ROOM2,Floor(0.6*Float(Room2Amount)),min_pos,max_pos)
	SetRoom("room1123",ROOM2,Floor(0.7*Float(Room2Amount)),min_pos,max_pos)
	SetRoom("room2elevator",ROOM2,Floor(0.85*Float(Room2Amount)),min_pos,max_pos)
	
	
	MapRoom(ROOM3, Floor(Rnd(0.2,0.8)*Float(Room3Amount))) = "room3storage"
	
	MapRoom(ROOM2C, Floor(0.5*Float(Room2CAmount))) = "room1162"
	...
	SetRoom("room079", ROOM1, Room1Amount[0]+Floor(0.15*Float(Room1Amount)),min_pos,max_pos)
	MapRoom(ROOM1, Room1Amount[0]+Floor(0.3*Float(Room1Amount[1]))) = "room106"
    SetRoom("008", ROOM1, Room1Amount[0]+Floor(0.4*Float(Room1Amount)),min_pos,max_pos)
    SetRoom("room035", ROOM1, Room1Amount[0]+Floor(0.5*Float(Room1Amount)),min_pos,max_pos)
    MapRoom(ROOM1, Room1Amount[0]+Floor(0.5*Float(Room1Amount[1]))) = "coffin"
	
	min_pos = Room2Amount[0]
	max_pos = Room2Amount[0]+Room2Amount[1]-1
	
	MapRoom(ROOM2, Room2Amount[0]+Floor(0.1*Float(Room2Amount))) = "room2nuke"
	SetRoom("room2tunnel", ROOM2, Room2Amount[0]+Floor(0.25*Float(Room2Amount)),min_pos,max_pos)
	SetRoom("room049", ROOM2, Room2Amount[0]+Floor(0.4*Float(Room2Amount)),min_pos,max_pos)
	;SetRoom("room2shaft",ROOM2,Room2Amount[0]+Floor(0.6*Float(Room2Amount[1])),min_pos,max_pos)
	MapRoom(ROOM2, Room2Amount[0]+Floor(0.7*Room2Amount[1])) = "room2servers"
	MapRoom(ROOM2, Room2Amount[0]+Floor(0.9*Float(Room2Amount[1]))) = "testroom"
	
	MapRoom(ROOM3, Room3Amount[0]+Floor(0.3*Float(Room3Amount))) = "room513"
	;MapRoom(ROOM3, Room3Amount[0]+Floor(0.6*Float(Room3Amount))) = "room966"
	...
	SetRoom("room2cafeteria", ROOM2, min_pos+Floor(0.2*Float(Room2Amount)),min_pos,max_pos)
	MapRoom(ROOM2, Room2Amount[0]+Room2Amount[1]+Floor(0.35*Float(Room2Amount[2]))) = "room2sroom"
	SetRoom("room2servers2", ROOM2, min_pos+Floor(0.4*Room2Amount[2]),min_pos,max_pos)	
	MapRoom(ROOM2, Room2Amount[0]+Room2Amount[1]+Floor(0.5*Room2Amount[2])) = "room2offices"
	...
	SetRoom("medibay", ROOM2, min_pos+Floor(0.7*Float(Room2Amount)),min_pos,max_pos)
	SetRoom("room2poffices2", ROOM2, min_pos+Floor(0.8*Room2Amount[2]),min_pos,max_pos)
	MapRoom(ROOM2, Room2Amount[0]+Room2Amount[1]+Floor(0.9*Float(Room2Amount[2]))) = "room2offices2"
	
	MapRoom(ROOM2C, Room2CAmount[0]+Room2CAmount[1]) = "room2ccont"	
	MapRoom(ROOM2C, 2) = "lockroom2"		
	
	MapRoom(ROOM3, Room3Amount[0]+Room3Amount[1]+Floor(0.3*Float(Room3Amount))) = "room3servers"
	MapRoom(ROOM3, Room3Amount[0]+Room3Amount[1]+Floor(0.7*Float(Room3Amount))) = "room3servers2"
	;MapRoom(ROOM3, Room3Amount[0]+Room3Amount[1]) = "room3gw"
	MapRoom(ROOM3, Room3Amount[0]+Room3Amount[1]+Floor(0.5*Float(Room3Amount))) = "room3offices"
