...
Function UpdateMainMenu()
	...
	Else
		...
		Select MainMenuTab
			Case 1 ; New game
				...
				Color 255,255,255
				If SelectedMap = "" Then
					AAText (x + 20 * MenuScale, y + 200 * MenuScale, "Map seed:")
					RandomSeed = Left(InputBox(x+150*MenuScale, y+195*MenuScale, 200*MenuScale, 30*MenuScale, RandomSeed, 3),15)	
				Else
					AAText (x + 20 * MenuScale, y + 200 * MenuScale, "Selected map:")
					Color (255, 255, 255)
					Rect(x+150*MenuScale, y+195*MenuScale, 200*MenuScale, 30*MenuScale)
					Color (0, 0, 0)
					Rect(x+150*MenuScale+2, y+195*MenuScale+2, 200*MenuScale-4, 30*MenuScale-4)
					
					Color (255, 0,0)
					If Len(SelectedMap)>15 Then
						AAText(x+150*MenuScale + 100*MenuScale, y+195*MenuScale + 15*MenuScale, Left(SelectedMap,14)+"...", True, True)
					Else
						AAText(x+150*MenuScale + 100*MenuScale, y+195*MenuScale + 15*MenuScale, SelectedMap, True, True)
					EndIf
					
					If DrawButton(x+370*MenuScale, y+195*MenuScale, 120*MenuScale, 30*MenuScale, "Deselect", False) Then
						SelectedMap=""
					EndIf
				EndIf	
				
				AAText(x + 20 * MenuScale, y + 160 * MenuScale, "Enable intro sequence:")
				;AAText(x + 100 * MenuScale, y + 160 * MenuScale, "Beware the soft locking guard D:")
				IntroEnabled = DrawTick(x + 280 * MenuScale, y + 160 * MenuScale, IntroEnabled)	
				
				;Local modeName$, modeDescription$, selectedDescription$
				AAText (x + 20 * MenuScale, y + 60 * MenuScale, "Difficulty:")				
				For i = SAFE To KETER
					If DrawTick(x + 20 * MenuScale, y + (90+30*i) * MenuScale, (SelectedDifficulty = difficulties(i))) Then SelectedDifficulty = difficulties(i)
					Color(difficulties(i)\r,difficulties(i)\g,difficulties(i)\b)
					AAText(x + 60 * MenuScale, y + (90+30*i) * MenuScale, difficulties(i)\name)
				Next
				...
				If DrawButton(x+20*MenuScale, y+245*MenuScale, 200*MenuScale, 30*MenuScale, "Load map", False) Then
					MainMenuTab = 4
					LoadSavedMaps()
				EndIf
	...
	AAText 20,GraphicHeight-90,"SCP - Containment Breach Classic Mod Reborn "+ModNumber$
	
	AASetFont ConsoleFont
	AAText 20,GraphicHeight-70,"Developed by xMYSTERKAPELUSZx"
	
	AASetFont ConsoleFont
	AAText 20,GraphicHeight-50,"Running on SCP: Containment Breach v"+VersionNumber
	
	AASetFont ConsoleFont
	AAText 20,GraphicHeight-30,"Made with Blitz3D v1.108 (Modified) and IDEal"
...
Function UpdateLauncher()
	...
	Repeat
		...
		;-----------------------------------------------------------------
		Color 255, 255, 255		
		Fullscreen = DrawTick(40 + 430, 262, Fullscreen)
		LauncherEnabled = DrawTick(40 + 430, 400, LauncherEnabled)
		
		Color 255, 255, 255
		Text(40 + 430 + 35, 262, "Fullscreen")
		Text(40 + 430 + 35, 400, "Use launcher")
		
		If DrawButton(LauncherWidth - 30 - 590, LauncherHeight - 45, 100, 30, "LAUNCH", False,False,False) Then
		...
		If DrawButton(LauncherWidth - 30 - 90, LauncherHeight - 45, 100, 30, "EXIT", False,False,False) Then End
		Flip
	Forever
...
Function DrawLoading(percent%, shortloading=False)
	...
	Repeat 
		...
		If percent = 100 Then 
			If firstloop And SelectedLoadingScreen\title <> "CWM" Then PlaySound_Strict(HorrorSFX(8)) ;LoadTempSound(("SFX\Horror\Horror8.ogg"))
