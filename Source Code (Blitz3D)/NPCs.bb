...
;								EndIf
			...
			Case NPCtype008
				...
				PositionEntity n\obj,EntityX(n\Collider,True),EntityY(n\Collider,True)-0.1,EntityZ(n\Collider,True)
				...
				Else
					...
					Else
						EntityAlpha n\obj,0.5+(Sin(AnimTime(n\obj)*6.0)/2.0)
					EndIf
				EndIf
				...
				If n\State>0 Then
					...
					If (Not EntityVisible(n\Collider,Collider)) Then
						...
						ElseIf n\PathStatus = 1
							...
							Else
								pvt = CreatePivot()
								PositionEntity pvt,EntityX(n\Collider),EntityY(n\Collider),EntityZ(n\Collider)
								PointEntity pvt,n\Path[n\PathLocation]\obj
								RotateEntity n\Collider, 0, CurveAngle(EntityYaw(pvt), EntityYaw(n\Collider), 10.0), 0
								FreeEntity pvt
								n\CurrSpeed = CurveValue(n\Speed,n\CurrSpeed,10.0)
								
								Animate2(n\obj, AnimTime(n\obj), 0, 14, n\CurrSpeed*10.0)
					...
					EndIf
					If dist<0.4 Then
						If (n\State2>=35.0) And (KillTimer>=0) Then
							Injuries=Injuries+Rnd(0.4,0.8)
							If Injuries>=4.0 Then
								If n\State3=0.0 Then
									DeathMSG = "Subject D-9341 was found dead in [REDACTED]. "
								...
								Kill()
							EndIf
							PlaySound_Strict DamageSFX(Rand(0,1))
							n\State2=0.0
						Else
							n\State2=n\State2+FPSfactor
						EndIf
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		...
		Default 
			CreateConsoleMsg("NPC type not found.") : Return;, 255, 0, 0) : Return
	End Select
