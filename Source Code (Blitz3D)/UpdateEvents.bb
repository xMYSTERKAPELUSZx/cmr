		...
		Select e\EventName
				...
				Else
					...
					If e\EventState > 500 Then
						...
						If e\EventState < 2000 Then
							...
							End If
							If Rand(600) = 1 Then PlaySound_Strict(IntroSFX(Rand(7, 9)))
							If Rand(400) = 1 Then PlaySound_Strict(IntroSFX(Rand(13, 14)))
						Else
							If Rand(1200) = 1 Then PlaySound_Strict(IntroSFX(Rand(7, 9)))
							If Rand(800) = 1 Then PlaySound_Strict(IntroSFX(Rand(13, 14)))
						EndIf
						
						If e\EventState > 900 And e\EventState - FPSfactor <= 900 Then e\SoundCHN2 = PlaySound_Strict(AlarmSFX(1))
						If e\EventState > 2000 And e\EventState - FPSfactor <= 2000 Then PlaySound_Strict(IntroSFX(7))
			...
			Case "173" ;the intro sequence
				...
				If KillTimer >= 0 And e\EventState2 = 0 Then
					...
					If e\EventState3>0 Then
						...
						ElseIf e\EventState3 => 150.0 And e\EventState3 < 700
							If e\room\NPC[3] = Null Then
								...
								e\Sound = LoadSound_Strict("SFX\Room\Intro\guard1.ogg")
							...
							If Distance(EntityX(Collider),EntityZ(Collider),PlayerRoom\x-3072*RoomScale, PlayerRoom\z+192.0*RoomScale)>1.5 Then
								If e\EventState3 > 250 Then
									...
									EndIf
									e\Sound = LoadSound_Strict("SFX\Room\Intro\guard4.ogg")
									PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
							...
							Else
								If e\EventState3-(FPSfactor/4) < 350 And e\EventState3=>350 Then
									e\Sound = LoadSound_Strict("SFX\Room\Intro\guard2.ogg")
								...
								ElseIf e\EventState3-(FPSfactor/4) < 550 And e\EventState3=>550 
									e\Sound = LoadSound_Strict("SFX\Room\Intro\guard3.ogg")
									e\room\NPC[3]\SoundChn = PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
						...
						ElseIf e\EventState3 < 900
							...
							Else
								...
								If e\room\NPC[3]\State3 => 70*8 And e\room\NPC[3]\State3-FPSfactor < 70*8 And e\room\NPC[3]\State=7 Then
									If e\room\NPC[3]\State2 < 2 Then
										e\Sound = LoadSound_Strict("SFX\Room\Intro\guard6.ogg")
										PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
										e\room\NPC[3]\State3=50
										e\room\NPC[3]\State2=3
									ElseIf e\room\NPC[3]\State2=3
										e\Sound = LoadSound_Strict("SFX\Room\Intro\guard7.ogg")
										PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
										e\room\NPC[3]\State3=50
										e\room\NPC[3]\State2=4
									ElseIf e\room\NPC[3]\State2=4
										e\Sound = LoadSound_Strict("SFX\Room\Intro\guard8.ogg")
										PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
								...
								Else
									...
									If dist > 5.5 Then
										...
										If e\room\NPC[3]\State2=0 Then
											e\Sound = LoadSound_Strict("SFX\Room\Intro\guard5.ogg")
											PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
							...
							If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\obj), EntityZ(e\room\obj)) < 8.0 Then
								e\Sound = LoadSound_Strict("SFX\Room\Intro\guard10.ogg")
								PlaySound2(e\Sound, Camera, e\room\NPC[3]\Collider)
					...
					Else
						...
						If e\EventState = 0 Then
							If PlayerRoom = e\room Then
								For i = 0 To 3
									IntroSFX(i) = LoadSound_Strict("SFX\Room\Intro\intro" + (i + 1) + ".ogg")
								Next
								For i = 4 To 6
									IntroSFX(i) = LoadSound_Strict("SFX\Room\Intro\refuse" + (i - 3) + ".ogg")
								Next
						...
						ElseIf e\EventState < 10000
							...
							ElseIf e\EventState >= 130 And e\EventState - (FPSfactor/3) < 130
								e\SoundCHN = PlaySound_Strict(IntroSFX(0))
						...
						ElseIf e\EventState < 14000 ; pelaaja sisällä huoneessa
							...
							ElseIf e\EventState > 10440 And e\EventState - FPSfactor < 10440 ;ovi aukee itsestään
								UseDoor(e\room\RoomDoors[1],False)
								e\SoundCHN = PlaySound_Strict(IntroSFX(7)) ;pamahdus
				...
				Else
					If KillTimer<0 Then
						If e\room\NPC[3]\State = 1 Then 
							e\Sound = LoadSound_Strict("SFX\Room\Intro\guard9.ogg")
			...
			Case "room2pipes106"
				...
				If (Not Contained106) Then 
					...
					Else
						...
						If (e\EventState / 250.0) > 0.1 And ((e\EventState - FPSfactor*0.7) / 250.0) <= 0.1 Then
...
;						EndIf
			...
			Case "room2test1074"
				...
				If PlayerRoom=e\room Then
					...
					If e\Sound=0 Then e\Sound=LoadSound_Strict("SFX\music\Intro.ogg")					
				...
				If e\EventState2 > 0 Then
					If e\EventState2 > 350 Then
						If e\EventState2 < 360 Then ;kun ovi avataan ensimmäisen kerran, toistetaan ääni
							...
							If RemoteDoorOn Then 	
								e\Sound = LoadSound_Strict("SFX\SCP\079\GateB.ogg")
								e\SoundCHN = PlaySound_Strict (e\Sound)						
								e\EventState2 = 400
